...
while True:
    ...
    frame2gray = cv2.cvtColor(frame_crop,cv2.COLOR_BGR2GRAY)
    hsv = cv2.cvtColor(frame_crop, cv2.COLOR_BGR2HSV)
    
    

    
    lower = np.array([26,96,52])
    upper = np.array([123,255,114])
    mask = cv2.inRange(hsv, lower, upper)
    res = cv2.bitwise_and(frame_crop,frame_crop, mask= mask)
    ...
    if len(contours)>0 :
        ...
        box = np.int0(box)
        cv2.drawContours(frame_crop,[box],0,(0,0,255),2)
